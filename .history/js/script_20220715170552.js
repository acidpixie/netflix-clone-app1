...
let movies = new Vue({
   ...
   computed: {
        ...
        filterItem() {
            return this.movies.movieName.filter((item) => {
                return item.movieName.toLowerCase().match(this.search.toLowerCase())
                || item.movieActors.toLowerCase().match(this.search.toLowerCase())
            });
        }
   },
